...
enum CustomerProjectDetailView {
  PROJECT_DETAIL,
  PROJECT_CHAT,
}
const CustomerProjectDetail = ({
  ...
  const [currentView, setCurrentView] = useState<CustomerProjectDetailView>(
    CustomerProjectDetailView.PROJECT_DETAIL
  );
  ...
  const renderProjectDetailSubNav = () => {
    return (
      <List>
        <ListItem
          onClick={() => {
            setCurrentView(CustomerProjectDetailView.PROJECT_DETAIL);
          }}
          sx={{
            borderRadius: "4px",
            ":hover": {
        ...
        >
          <Typography
            variant={
              currentView === CustomerProjectDetailView.PROJECT_DETAIL
                ? "subtitle2"
                : "caption"
            }
          >
            {intl.formatMessage({
              id: "app.customer.projectDetail.subnav.project",
            })}
          </Typography>
        </ListItem>
        <ListItem
          onClick={() => {
            setCurrentView(CustomerProjectDetailView.PROJECT_CHAT);
          }}
          sx={{
            borderRadius: "4px",
            ":hover": {
        ...
        >
          <Typography
            variant={
              currentView === CustomerProjectDetailView.PROJECT_CHAT
                ? "subtitle2"
                : "caption"
            }
          >
            {intl.formatMessage({
              id: "app.customer.projectDetail.subnav.messages",
            })}
          </Typography>
        </ListItem>
      </List>
  ...
  if (projectData && projectChangelogData) {
    return (
      <Container maxWidth="xl">
        ...
        <Box display="flex">
          {!!projectData.bids && !!projectData.bids.length && (
            <Box sx={{ flexGrow: 0, mr: 1, flexBasis: "10%" }}>
              {renderProjectDetailSubNav()}
            </Box>
          )}
                          ...
                          !!getProjectChangelogData.getProjectChangelog
                            ...
                            >
                              <Button
                                variant="outlined"
                                onClick={() =>
                                  setProjectChangelogModalOpen(true)
                                }
                              >
                                {" "}
                                {intl.formatMessage({
                              ...
                              <Dialog
                                ...
                                onClose={() =>
                                  setProjectChangelogModalOpen(false)
                                }
                                maxWidth="lg"
                                // fullWidth
                              >
                      ...
                      <Box sx={{ position: "relative" }}>
                        {!!componentsChangelog[comp.id] && (
                          ...
                          >
                            <Button
                              onClick={() =>
                                setComponentChangelogModalOpen(true)
                              }
                              variant="outlined"
                            >
                              {intl.formatMessage({
          ...
          {currentView === CustomerProjectDetailView.PROJECT_CHAT && (
            <Box sx={{ flexGrow: 2 }}>
              <CustomerChat bids={projectData.bids ? projectData.bids : []} />
            </Box>
          )}
        </Box>
      </Container>
