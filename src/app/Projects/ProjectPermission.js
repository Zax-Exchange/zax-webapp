...
import { Autocomplete, Button, Container, Typography, Card, Input, List, ListItem, Select, MenuItem, ListItemButton, TextField } from "@mui/material";
...
const ProjectPermission = ({ projectData, setIsPermissionOpen, isVendor }) => {
  ...
  const {error: projectUserDataError, loading: projectUserDataLoading, data: projectUserData} = useQuery(GET_PROJECT_USERS, {
    variables: {
  ...
  const {error: bidError, loading: bidLoading, data: bidData} = useQuery(GET_PROJECT_BID_USERS, {
    variables: {
    ...
    if (userData && allCompanyUsersData && bidData) {
      if (isVendor) {
        allCompanyUsersData.getAllUsersWithinCompany.forEach((data) => {
          ...
          if (!bidData.getProjectBidUsers.find((user) => user.email == data.email)) {
  ...
  const isUserWithinPermission = (userId) => {
    if (isVendor) {
      return bidData.getProjectBidUsers.findIndex(user => user.id === userId) >= 0;
    } else {
      return projectUserData.getProjectUsers.findIndex(user => user.id === userId) >= 0;
    }
  }
  const deleteViewersEditorsList = (userId, permission) => {
    ...
    console.log(isUserWithinPermission(userId))
    if (toDelete.indexOf(userId) === -1 && isUserWithinPermission(userId)) {
      // check if user is within project/bid permission already
    ...
    return <List>
      {
        allProjectUsers && allProjectUsers.map(data => {
          ...
          return <ListItem key={data.email}>
              ...
              <Select label="Permission" sx={{width: 200}} onChange={(e) => selectPermissionHandler(e, data.userId)} value={data.permission}>
